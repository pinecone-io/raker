...
impl RakerClient {
    ...
    pub async fn create_curate_task(&self, context_id: &str) -> Result<serde_json::Value> {
        let resp = self
            .http
            .post(self.api(&format!("/contexts/{context_id}/knowledge/curate")))
            .header("Authorization", self.auth_header()?)
            .send()
            .await?;
        let resp = Self::check(resp).await?;
    ...
    pub async fn create_build_task(&self, context_id: &str) -> Result<serde_json::Value> {
        let resp = self
            .http
            .post(self.api(&format!("/contexts/{context_id}/code/build")))
            .header("Authorization", self.auth_header()?)
    ...
    pub async fn upload_file(
        ...
        let mut url = self.api(&format!("/contexts/{context_id}/source/import/upload"));
        if let Some(tp) = target_path {
            url = format!("{}?path={}", url, tp);
        }
